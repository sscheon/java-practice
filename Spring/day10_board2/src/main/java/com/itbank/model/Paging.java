...
public class Paging {
	...
	private int pageCount;		// 만약 게시글이 253개이면, 페이지 개수는 26
	...
	public Paging(int page, int boardCount) {	// 생성자(요청받은 페이지, 총 글수)
		...
		// 현재 보고있는 페이지가 13이고, begin은 11이면 prePage는 10이므로 이전 섹션이 존재함
		
		next = lastSection > section;
		// 마지막 섹션이 현재 섹션보다 더 큰값이면 [다음]을 출력한다
		// 내가보는 페이지는 21, 마지막 값은 40이면 다음을 출력한다
...
//			str += "요청 받은 페이지 : %d\n";
//			str += "요청 받은 페이지 : %d\n";
//			str = String.format(str, page, boardCount, section, begin, end, prev, next);
//			return str;
//		}
