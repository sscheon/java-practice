...
// 인터셉터 : 요청을 가로채서, 사전 코드를 실행할 수 있다. 여러 주소에 대해 일괄 적용 가능
...
//		response.getWriter().append("<h1>이 페이지는 이제 제껍니다</h1>");
//		response.sendRedirect(request.getContextPath() + "/");
		...
		url = URLEncoder.encode(url, "utf-8");	// https://	주소창에 쓸수있는 방식으로 변환 (특수기호, 한글, 공백문자)
		
		if(login == null ) {
			System.out.println("인터셉터어 의해 로그인 페이지로 이동합니다");
