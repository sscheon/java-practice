...
class PowerChargeCable extends Object implements USB { // 전력 전송
...
class Speaker implements USB { // 음성 데이터 출력
...
public class Ex04 {
	public static void main(String[] args) {
		...
		pc.connect(cable);	// 케이블 연결 시 반응이 정상적으로 호출됨
		pc.connect(ssd);	// SSD 연결 시 반응이 정상적으로 호출됨
		pc.connect(sp);		// 스피커 연결 시 반응이 정상적으로 호출됨
		
		// Java 8 이후 함수형 인터페이스 형식이 추가되었음
		// 함수형 인터페이스란, 단 하나의 추상메서드만 가지는 인터페이스를 말한다
		// 함수형 인터페이스는 람다식에 의해 객체를 생성할 수 있다
		// 익명클래스에 의한 객체 생성보다 식이 간편하고, 내부 작동도 다르다
		...
		// ex) Interface ob = a -> a > 0 ? a : -a;
		//	   Interface ob = (a) -> { return a > 0 ? a : -a };
		// 특정함수를 객체화하기 위해서 interface를 쓴다
	}
}
