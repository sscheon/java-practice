...
<ol>
	<li>JSP에서 파일을 전송받으면, 일반 parameter로 처리할 수 없다 (반환형이 String)</li>
	<li>JSP 내장객체 request에서는 파일을 처리하는 함수가 준비되어 있지 않다</li>
	<li>파일 처리를 위한 별도의 라이브러리를 가져와서 처리한다 (com.oreilly.servlets)</li>
</ol>
<%@ page import="com.oreilly.servlet.MultipartRequest" %>
<%
	// 내장객체 request로는 파일을 처리할 수 없어서, MultipartRequest 타입의 객채로 변환한다
	...
	String fileNmae = uploadFile.getName();				// file의 원래 이름 (String)
	// file은 웹 서버 또는 별도의 서버에 저장하지만, DB에는 파일의 이름을 문자열로 기록해야 하기 때문에
	// 파일도 확보해야하고, 파일이름도 확보해야한다
